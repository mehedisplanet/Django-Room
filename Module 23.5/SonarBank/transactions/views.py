...
class MoneyTransferView(FormView):
    ...
    def form_valid(self, form):
        ...
        sender_message = f'Successfully sent {"{:,.2f}".format(float(transfer_amount))}$ from {(recipient_account)} account.'
        messages.success(self.request, sender_message)

        send_transaction_email(sender_account.user, transfer_amount, "Money Transfer Sent", 'transactions/money_transfer_email.html')
        send_transaction_email(recipient_account.user, transfer_amount, "Money Transfer Received", 'transactions/money_received_email.html')
